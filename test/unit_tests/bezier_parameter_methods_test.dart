...
void main() {
  group('pointAt', () {
    test('quadratic pointAt', () {
      final curve =
          QuadraticBezier([Vector2(10.0, 10.0), Vector2(70.0, 95.0), Vector2(15.0, 80.0)]);
      ...
      expect(
          curve.pointAt(0.33333333), closeToVector(Vector2(37.22222137451172, 55.5555534362793)));
      expect(curve.pointAt(0.85), closeToVector(Vector2(28.912500381469727, 82.25)));
    });

    test('cubic pointAt', () {
      final curve = CubicBezier(
          [Vector2(10.0, 10.0), Vector2(70.0, 95.0), Vector2(25.0, 20.0), Vector2(15.0, 80.0)]);
      ...
      expect(curve.pointAt(0.85), closeToVector(Vector2(21.389999389648438, 61.11687469482422)));
    });
  });
  ...
  group('hullPointsAt', () {
    ...
    test('cubic hullPointsAt', () {
      final curve = CubicBezier(
          [Vector2(0.0, 0.0), Vector2(0.0, 100.0), Vector2(50.0, -100.0), Vector2(100.0, 0.0)]);
    ...
    test('cubic hullPointsAt, second test', () {
      ...
      expect(hull2[7], closeToVector(Vector2(38.79999923706055, 43.599998474121094)));
      expect(hull2[8], closeToVector(Vector2(20.399999618530273, 61.400001525878906)));
      expect(hull2[9], closeToVector(Vector2(24.079999923706055, 57.84000015258789)));
    });

    test('quadratic hullPointsAt, second test', () {
      final points = [Vector2(10.0, 10.0), Vector2(25.0, 20.0), Vector2(15.0, 80.0)];
  ...
  group('derivativeAt', () {
    ...
    test('cubic derivativeAt', () {
      ...
      expect(
          curve.derivativeAt(0.1), closeToVector(Vector2(121.19999694824219, 167.85000610351562)));
      ...
      expect(
          curve.derivativeAt(0.9), closeToVector(Vector2(-46.79999923706055, 107.8499984741211)));
    ...
    test('quadratic derivativeAt, specified cachedFirstOrderDerivativePoints', () {
      ...
      final cachedPoints = <Vector2>[Vector2(120.0, 170.0), Vector2(-110.0, -30.0)];

      expect(curve.firstOrderDerivativePoints, closeToVectorList(cachedPoints));

      expect(curve.derivativeAt(0.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(120.0, 170.0)));
      expect(curve.derivativeAt(0.1, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(97.0, 150.0)));
      expect(curve.derivativeAt(0.25, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(62.5, 120.0)));
      expect(curve.derivativeAt(0.33333, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(43.33409881591797, 103.33399963378906)));
      expect(curve.derivativeAt(0.5, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(5.0, 70.0)));
      expect(curve.derivativeAt(0.6827, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-37.020999908447266, 33.459999084472656)));
      expect(curve.derivativeAt(0.75, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-52.5, 20.0)));
      expect(curve.derivativeAt(0.9, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-87.0, -10.0)));
      expect(curve.derivativeAt(1.0, cachedFirstOrderDerivativePoints: cachedPoints),
  ...
  group('normalAt', () {
    test('cubic normalAt', () {
      ...
      expect(curve.normalAt(0.5), closeToVector(Vector2(0.12403473258018494, -0.9922778606414795)));
      expect(curve.normalAt(0.23), closeToVector(Vector2(-0.8163508176803589, 0.5775563716888428)));
      expect(
          curve.normalAt(0.69), closeToVector(Vector2(-0.24693183600902557, -0.9690328240394592)));
      expect(curve.normalAt(0.0), closeToVector(Vector2(-0.8169678449630737, 0.5766832232475281)));
      expect(curve.normalAt(1.0), closeToVector(Vector2(-0.986393928527832, -0.16439898312091827)));
    });
    ...
    test('quadratic normalAt', () {
      ...
      expect(curve.normalAt(0.5), closeToVector(Vector2(-0.9974586963653564, 0.07124704867601395)));
      expect(curve.normalAt(0.23), closeToVector(Vector2(-0.9185916185379028, 0.3952080309391022)));
      expect(
          curve.normalAt(0.69), closeToVector(Vector2(-0.9987242221832275, -0.05049728974699974)));
      expect(curve.normalAt(0.0), closeToVector(Vector2(-0.5547001957893372, 0.8320503234863281)));
      expect(curve.normalAt(1.0), closeToVector(Vector2(-0.986393928527832, -0.16439898312091827)));
    });

    test('cubic normalAt, specified cachedFirstOrderDerivativePoints', () {
      final curve = CubicBezier(
          [Vector2(0.0, 0.0), Vector2(30.0, 100.0), Vector2(80.0, -30.0), Vector2(95.0, 45.0)]);
      ...
      expect(curve.normalAt(0.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-0.9578262567520142, 0.2873478829860687)));
      expect(curve.normalAt(0.18, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-0.6621309518814087, 0.7493881583213806)));
      expect(curve.normalAt(0.375, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(0.29084232449531555, 0.9567710161209106)));
      expect(curve.normalAt(0.5, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(0.5057194828987122, 0.862697958946228)));
      expect(curve.normalAt(0.625, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(0.4644697606563568, 0.8855890035629272)));
      expect(curve.normalAt(0.82, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-0.5096653699874878, 0.8603727221488953)));
      expect(curve.normalAt(1.0, cachedFirstOrderDerivativePoints: cachedPoints),
    ...
    test('quadratic normalAt, specified cachedFirstOrderDerivativePoints', () {
      final curve =
          QuadraticBezier([Vector2(130.0, -40.0), Vector2(110.0, 85.0), Vector2(-45.0, 170.0)]);
  ...
  group('offsetPointAt', () {
    test('cubic offsetPointAt', () {
      ...
      expect(pointA, closeToVector(Vector2(37.30337142944336, 62.81601333618164)));

      final pointB = curve.offsetPointAt(0.85, 10.0);
      expect(pointB, closeToVector(Vector2(34.19104766845703, 39.19478988647461)));

      final pointC = curve.offsetPointAt(0.0, 16.0);
      expect(pointC, closeToVector(Vector2(-3.0714855194091797, 19.226932525634766)));

      final pointD = curve.offsetPointAt(1.0, 1.0);
      expect(pointD, closeToVector(Vector2(25.980581283569336, 20.196115493774414)));
    });
    ...
    test('quadratic offsetPointAt', () {
      ...
      expect(pointA, closeToVector(Vector2(48.78436279296875, 62.17439651489258)));

      final pointB = curve.offsetPointAt(0.15, 5.0);
      expect(pointB, closeToVector(Vector2(31.915807723999023, 61.86527633666992)));

      final pointC = curve.offsetPointAt(0.0, 1.0);
      expect(pointC, closeToVector(Vector2(15.707106590270996, 70.70710754394531)));

      final pointD = curve.offsetPointAt(1.0, 8.0);
      expect(pointD, closeToVector(Vector2(62.01771926879883, 95.53215026855469)));
    });

    test('quadratic offsetPointAt, negative distance', () {
      final curve = QuadraticBezier([Vector2(0.0, 0.0), Vector2(50.0, 100.0), Vector2(100.0, 0.0)]);

      expect(curve.offsetPointAt(0.5, -1.0), closeToVector(Vector2(50.0, 49.0)));
      expect(curve.offsetPointAt(0.5, -50.0), closeToVector(Vector2(50.0, 0.0)));
    ...
    test('cubic offsetPointAt, negativeDistance', () {
      final curve = CubicBezier(
          [Vector2(0.0, 100.0), Vector2(250.0, 0.0), Vector2(-150.0, 50.0), Vector2(100.0, 100.0)]);
    ...
    test('quadratic offsetPointAt, specified cachedFirstOrderDerivativePoints', () {
      ...
      final cachedPoints = <Vector2>[Vector2(100.0, 200.0), Vector2(100.0, -200.0)];

      expect(curve.firstOrderDerivativePoints, closeToVectorList(cachedPoints));

      expect(curve.offsetPointAt(0.0, 10.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-8.9442720413208, 4.4721360206604)));
      expect(curve.offsetPointAt(0.25, 40.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(-3.284270763397217, 65.78427124023438)));
      expect(curve.offsetPointAt(0.5, 24.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(50.0, 74.0)));
      expect(curve.offsetPointAt(0.5, -30.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(50.0, 20.0)));
      expect(curve.offsetPointAt(0.55, -30.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(49.11651611328125, 20.082578659057617)));
      expect(curve.offsetPointAt(0.83, 40.0, cachedFirstOrderDerivativePoints: cachedPoints),
          closeToVector(Vector2(114.88368225097656, 52.374305725097656)));
      expect(curve.offsetPointAt(1.0, -10.0, cachedFirstOrderDerivativePoints: cachedPoints),
  ...
  group('positionLookUpTable', () {
    ...
    test('cubic positionLookUpTable, default intervalsCount', () {
      final curve = CubicBezier(
          [Vector2(0.0, 0.0), Vector2(50.0, 100.0), Vector2(50.0, 0.0), Vector2(100.0, 100.0)]);
      ...
      expect(table[0], closeToVector(Vector2(0.0, 0.0)));
      expect(table[1], closeToVector(Vector2(2.9407999515533447, 5.763200283050537)));
      expect(table[20], closeToVector(Vector2(42.400001525878906, 49.60000228881836)));
      expect(table[25], closeToVector(Vector2(50.0, 50.0)));
      expect(table[30], closeToVector(Vector2(57.599998474121094, 50.39999771118164)));
      expect(table[49], closeToVector(Vector2(97.05919647216797, 94.23680114746094)));
