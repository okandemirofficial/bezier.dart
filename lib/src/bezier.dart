...
abstract class Bezier {
  ...
  Vector2 offsetPointAt(double t, double distance,
    ...
    final normalVector =
        normalAt(t, cachedFirstOrderDerivativePoints: cachedFirstOrderDerivativePoints);
  ...
  Vector2 get _scalingOrigin {
    final firstOrderPoints = firstOrderDerivativePoints;
    final offsetStart =
        _nonOverlappingOffsetPointAt(0.0, originIntersectionTestDistance, 0, firstOrderPoints);
    final offsetEnd =
        _nonOverlappingOffsetPointAt(1.0, originIntersectionTestDistance, order, firstOrderPoints);
    final intersectionPoint =
        intersectionPointBetweenTwoLines(offsetStart, startPoint, offsetEnd, endPoint);
  ...
  Vector2 _nonOverlappingOffsetPointAt(
      double t, double distance, int endPointIndex, List<Vector2> cachedPoints) {
    final offsetPoint = pointAt(t);
  ...
  Bezier scaledCurve(double distance) {
    ...
    scaledCurvePoints.add(
        intersectionPointBetweenTwoLines(scaledStartPoint, startTangentPoint, origin, points[1]) ??
            startTangentPoint);
    ...
    if (order == 3) {
      ...
      scaledCurvePoints.add(
          intersectionPointBetweenTwoLines(scaledEndPoint, endTangentPoint, origin, points[2]) ??
              endTangentPoint);
    }
  ...
  static List<Intersection> _removeDuplicateIntersections(
      List<Intersection> intersectionsToFilter, double minTValueDifference) {
  ...
  static List<Intersection> _locateIntersections(List<BezierSlice> curve1, List<BezierSlice> curve2,
    ...
    for (var pairIndex = 0; pairIndex < overlappingSegmentsCount; pairIndex++) {
      ...
      final result =
          locateIntersectionsRecursively(leftCurve, rightCurve, curveIntersectionThreshold);
  ...
  List<Intersection> intersectionsWithCurve(Bezier curve,
    ...
    return _locateIntersections(
        reducedSegments, curveReducedSegments, curveIntersectionThreshold, minTValueDifference);
  }
  ...
  List<Intersection> intersectionsWithSelf(
    ...
    for (var segmentIndex = 0; segmentIndex < reducedSegments.length - 2; segmentIndex++) {
      ...
      final result =
          _locateIntersections(left, right, curveIntersectionThreshold, minTValueDifference);
